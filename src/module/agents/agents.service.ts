...
import { convertDate, parseTimeStringToSeconds, returnMothData, splitTextIntoChunks, subtractTime } from 'src/utils/converters';
...
import { ControlAgentGraph, ControlAgentGraphSmena } from 'src/utils/agentControlfunctions';
...
export class AgentsService {
  ...
  const findTime = await agentslockEntity.find({
    ...
    async findAgentGraph(login: string) {
      ...
      async findLockData(agentId: string ,pageNumber = 1, pageSize = 10 ,  fromDate: string , untilDate: string) {
      ...
      async findControlTgraphData(agent_id :string ,pageNumber = 1, pageSize = 10 , fromDate: string , untilDate: string ) {
        const offset = (pageNumber - 1) * pageSize;
        ...
        const  [results, total]  = await agentControlGraphEntity.findAndCount({
          where: [
            { id_login: findAgent.id_login, ComeToWorkOnTime:  Not(true), LeftAfterWork:  Not(true), TimeWorkIsDone:  Not(true) ,create_data: Between(fromDateFormatted, untilDateFormatted) },
            { id_login: findAgent.id_login, ComeToWorkOnTime: true, LeftAfterWork: true, TimeWorkIsDone: Not(true) ,create_data: Between(fromDateFormatted, untilDateFormatted) },
            { id_login: findAgent.id_login, ComeToWorkOnTime: true, LeftAfterWork: Not(true), TimeWorkIsDone: Not(true)  ,create_data: Between(fromDateFormatted, untilDateFormatted)},
            { id_login: findAgent.id_login, ComeToWorkOnTime: Not(true), LeftAfterWork: true, TimeWorkIsDone: Not(true)  ,create_data: Between(fromDateFormatted, untilDateFormatted)},
            { id_login: findAgent.id_login, ComeToWorkOnTime: Not(true), LeftAfterWork: Not(true), TimeWorkIsDone: true  ,create_data: Between(fromDateFormatted, untilDateFormatted)},
            { id_login: findAgent.id_login, ComeToWorkOnTime: true, LeftAfterWork: Not(true), TimeWorkIsDone: true ,create_data: Between(fromDateFormatted, untilDateFormatted) },
            { id_login: findAgent.id_login, ComeToWorkOnTime: Not(true), LeftAfterWork: true, TimeWorkIsDone: true  ,create_data: Between(fromDateFormatted, untilDateFormatted)},
        ...
        const  agentData = await agentControlGraphEntity.find({
          where: 
      ...
      async findComeToWorkOnTimeData(fromDate: string , untilDate: string) {
          ...
          const findAgents = await agentControlGraphEntity.find({
            where: {
          ...
          const agents = await this.findAllOperatorBanInfo(сomeToWorkLateAllOperators , fromDateFormatted, untilDateFormatted)
    ...
    async findallBanTimeData(fromDate: string , untilDate: string) {
      ...
      const findAgents = await agentslockEntity.find({
        where: {
          banInfo : 'time',
          create_data :Between(fromDateFormatted , untilDateFormatted)
        },
      ...
      const agents = await this.findAllOperatorBanInfoDateAgentLock(AllbanTimeOperators , fromDateFormatted, untilDateFormatted)
  ...
  async findAllOperatorBanInfoDateAgentLock(agentsArr : agentslockEntity[] , fromDateFormatted:Date ,untilDateFormatted :Date) {
    let  agents = [] 
      ...
      const findAgent = await AgentDateEntity.findOne( {
       where: {
         ...
         months : {
           days :{
             the_day_Format_Date : Between(fromDateFormatted , untilDateFormatted)
           }
         }
       },
       relations : {
     ...
     if(findAgent) {
      for(let  moth of findAgent.months ) {
        ...
        if(moth){
          for(let day of moth.days) {
            const typesTime = [
              "10-19", "07-16", "08-17", "09-18", "11-20", "13-22",
              "15-24", "17-02", "07-15", "08-16", "09-17", "08-18", "18-08", "14-23",
              "18-09", "09-18"
             ];
             const typesSmen = ["08-20", "20-08"]
             if(typesTime.includes(day.work_time) && day.work_type == 'day'){
               allworkTime +=9 
             }else if(typesSmen.includes(day.work_time) && day.work_type == 'smen' ) {
               allworkTime +=12 
             }
           }
        }
      ...
      const  CountAgentсomeToWorkLate = await agentControlGraphEntity.count({
        where : {
          id_login : e.login.toString(),
         ComeToWorkOnTime : false
        }
      ...
      const  CountAgentLeftAfterWork = await agentControlGraphEntity.count({
        where : {
          id_login : e.login.toString(),
          LeftAfterWork : false
        }
      ...
      const  CountAgentBlock = await agentslockEntity.count({
        where : {
          id: e.id,
          banInfo :'block' 
        }
      ...
      const  CountAgentBanTime = await agentslockEntity.count({
        where : {
          id: e.id,
          banInfo :'time' 
        }
      ...
      agents.push({
        agent_id: findAgent.agent_id , id: findAgent.id, service_name:findAgent.service_name, id_login: findAgent.id_login , name: findAgent.name,  create_data :findAgent.create_data , allworkTime , CountAgentсomeToWorkLate :CountAgentсomeToWorkLate , CountAgentLeftAfterWork ,  CountAgentBlock , CountAgentBanTime
      })
     }
  ...
  async findAllOperatorBanInfo(agentsArr : agentControlGraphEntity[] , fromDateFormatted:Date ,untilDateFormatted :Date) {
        let  agents = [] 
    ...
    async writeNewGraph() {
            ...
            if(findAgent) {
              ...
              // console.log(updateAgent.raw[0].agent_id ,'okkk');
                      ...
                      if(typesGraph.includes(dataDay[1])) {
                        ...
                        .set({ at_work : dataDay[1] , work_day : +dataDay[0].split('.')[0]  ,work_time :null , the_date : dataDay[0] , the_day_Format_Date : formatDate   , work_type : dataDay[1] , week_day_name: dataDay[2]})
                        ...
                        await GraphDaysEntity.createQueryBuilder()
                        .update(GraphDaysEntity)
                        .set({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0] ,work_time : dataDay[1] , the_date : dataDay[0] , the_day_Format_Date : formatDate     , work_type : 'day' , week_day_name: dataDay[2]})
                        .where("id = :id", {  id:findDay.id   })
                        .returning(['id'])
                        .execute()
                        ...
                        await GraphDaysEntity.createQueryBuilder()
                        .update(GraphDaysEntity)
                        .set({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0]  ,work_time : dataDay[1] , the_date : dataDay[0], the_day_Format_Date : formatDate     , work_type : 'smen' , week_day_name: dataDay[2]})
                        .where("id = :id", {  id:findDay.id   })
                        .returning(['id'])
                        .execute()
                      }
                      ...
                      if(typesGraph.includes(dataDay[1])) {
                       ...
                       await GraphDaysEntity.createQueryBuilder()
                        ...
                        .values({
                          at_work : dataDay[1] ,work_day : +dataDay[0].split('.')[0] ,work_time :null , the_date : dataDay[0] , the_day_Format_Date : formatDate    , work_type : dataDay[1] , week_day_name: dataDay[2], month_id :findMonth[0].id
                        }).returning(['id'])
                        .execute()
                        .catch((e) => {
                          throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
                       ...
                       await GraphDaysEntity.createQueryBuilder()
                        .insert()
                        .into(GraphDaysEntity)
                        .values({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0] ,work_time : dataDay[1] , the_date : dataDay[0], the_day_Format_Date : formatDate    , work_type : 'day' , week_day_name: dataDay[2] , month_id :findMonth[0].id})
                        .returning(['id'])
                        .execute()
                        .catch((e) => {
                          throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
                       ...
                       await GraphDaysEntity.createQueryBuilder()
                        .insert()
                        .into(GraphDaysEntity)
                        .values({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0] ,work_time : dataDay[1] , the_date : dataDay[0] , the_day_Format_Date : formatDate   , work_type : 'smen'  , week_day_name: dataDay[2],  month_id :findMonth[0].id})
                        .returning(['id'])
                        .execute()
                        .catch((e) => {
                          throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
                      ...
                      const typesSmen = ["08-20", "20-08"]
                        ...
                        if(typesGraph.includes(dataDay[1])) {
                         ...
                         await GraphDaysEntity.createQueryBuilder()
                          ...
                          .into(GraphDaysEntity)
                          .values({
                            at_work : dataDay[1] ,work_day : +dataDay[0].split('.')[0] ,work_time :null , the_date : dataDay[0] , the_day_Format_Date : formatDate    , work_type : dataDay[1], week_day_name: dataDay[2] , month_id :newMoth.raw[0].id
                          }).returning(['id'])
                          .execute()
                          .catch((e) => {
                            throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
                         ...
                         await GraphDaysEntity.createQueryBuilder()
                          .insert()
                          .into(GraphDaysEntity)
                          .values({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0] ,work_time : dataDay[1] , the_date : dataDay[0] ,the_day_Format_Date : formatDate     , work_type : 'day' , week_day_name: dataDay[2] , month_id :newMoth.raw[0].id})
                          .returning(['id'])
                          .execute()
                          .catch((e) => {
                            throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
                          ...
                          .insert()
                          .into(GraphDaysEntity)
                          .values({ at_work : 'W' ,work_day : +dataDay[0].split('.')[0] ,work_time : dataDay[1] , the_date : dataDay[0] ,the_day_Format_Date : formatDate    , work_type : 'smen' , week_day_name: dataDay[2] ,  month_id :newMoth.raw[0].id  })
                          .returning(['id'])
                          .execute()
                          .catch((e) => {
                            throw new HttpException('Bad Request', HttpStatus.BAD_REQUEST);
      ...
      //  console.log(theCurrentHour);
        ...
        if(6 <= theCurrentHour && theCurrentHour <= 8 || 15 <= theCurrentHour && theCurrentHour <= 17  ) {
          // console.log('if 1');
        ...
        if(7 <= theCurrentHour && theCurrentHour <= 9 || 16 <= theCurrentHour && theCurrentHour <= 18  ) {
          // console.log('if 2');
        ...
        if(8 <= theCurrentHour && theCurrentHour <= 10 || 17 <= theCurrentHour && theCurrentHour <= 19  ) {
        ...
        if(10 <= theCurrentHour && theCurrentHour <= 12 || 19 <= theCurrentHour && theCurrentHour <= 21  ) {
          // console.log('if 4');
        ...
        if(12 <= theCurrentHour && theCurrentHour <= 14 || 21 <= theCurrentHour && theCurrentHour <= 23  ) {
          // console.log('if 5');
        ...
        if(14 <= theCurrentHour && theCurrentHour <= 16 || 23 <= theCurrentHour && theCurrentHour <= 24  || 0 <= theCurrentHour && theCurrentHour <= 1  ) {
          // console.log('if 6');
        ...
        if(16 <= theCurrentHour && theCurrentHour <= 18 || 1 <= theCurrentHour && theCurrentHour <= 3   ) {
        ...
        if(7 <= theCurrentHour && theCurrentHour <= 9 || 19 <= theCurrentHour && theCurrentHour <= 21   ) {
          // console.log('if 8');
          ...
          //  console.log(findAgentinControlGraph, agentStatistic ,'4');
            ...
            //  console.log(agentStatistic); 
             if(findAgentinControlGraph.WorkState != 'did not work') {
                ...
                if(endWorkTimeParseSeconds <= fullDurationOfWorkTimeParseSeconds + startWorkTimeParseSeconds ) {
